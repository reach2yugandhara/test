...
Module Module1
    ...
    'Code for Sarvoisky with access database
    ...
    Sub Main()
        ...
        'Dim response2 As String = response.ToString("yyyy-MM-dd hh:mm:ss")
        'onsole.WriteLine("Response :: " + response2)
        ...
        Try
            ...
            cn.Open()
            Dim querystr As String = "SELECT id, rdate, type, cancel, billno, tax, discount, total, SaleTime,saleid,giftvoucher,grandtotal FROM sale WHERE id  > " + response + " ORDER BY id"
            ...
            While dr.Read()
                ...
                Dim DocNo As String = dr.GetValue(4).ToString

                Dim docDt As String = dr.GetDateTime(1).ToString("yyyy-MM-dd")
                Dim receiptNo As String = DocNo + "-" + docDt
                Dim docTime As String = dr.GetDateTime(8).ToString("hh':'mm':'ss")
                Dim tax As String = Convert.ToString(dr.GetValue(5))
                Dim gift As Decimal = 0
                If dr.GetValue(10) Is DBNull.Value Then
                    gift = 0
                Else
                    gift = Convert.ToDecimal(dr.GetValue(10))
                End If

                Dim totNetDocValue As String = Convert.ToDecimal(dr.GetValue(11))
                Dim transaction_id As String = Convert.ToString(dr.GetValue(0))
                Dim grossValue As String = Convert.ToString(Convert.ToDecimal(dr.GetValue(7)))
                Dim totDisc As String = Convert.ToString(dr.GetValue(6))
                Dim saleQtySum As Decimal = 0
                Dim pmode As String = ""
                Dim type As Boolean = Convert.ToBoolean(dr.GetValue(2))
                Dim cancel As Boolean = Convert.ToBoolean(dr.GetValue(3))
                Dim saleid As Integer = Convert.ToInt32(dr.GetValue(9))
                'Dim amount As Integer
                Dim returnFlag As String = "N"

                'If dr.GetValue(11) Is DBNull.Value Then
                '    amount = 0
                'Else
                '    amount = Convert.ToDecimal(dr.GetValue(11))
                'End If
                ...
                Dim query_1 As String = "Select ItemId,UniqueCode,Quantity,Tax,DiscPercent,DiscountAmount,Total,rate from saleChild where Saleid=" + DocNo + ""
                ...
                While dr1.Read()
                    ...
                    Dim lineItemName As String = dr1.GetValue(1).ToString
                    Dim lineQuantity As Decimal = Convert.ToDecimal(dr1.GetValue(2))
                    Dim TaxAmt As String = Convert.ToString(dr1.GetValue(3))
                ...
                If returnFlag = "N" Then
                    sales_record = receiptNo + SEPARATOR_FIELDS + docDt + SEPARATOR_FIELDS + docTime + SEPARATOR_FIELDS + grossValue + SEPARATOR_FIELDS + totNetDocValue.ToString() + SEPARATOR_FIELDS + tax.ToString() + SEPARATOR_FIELDS + totDisc.ToString() + SEPARATOR_FIELDS + saleQtySum.ToString() + SEPARATOR_FIELDS + transaction_id + SEPARATOR_FIELDS + tax.ToString + SEPARATOR_FIELDS + pmode + SEPARATOR_FIELDS + "1" + SEPARATOR_FIELDS
                    Console.WriteLine("SALES RECORD")
                ElseIf returnFlag = "Y" Then
                    receiptNo = receiptNo + "R"
                    sales_record = receiptNo + SEPARATOR_FIELDS + docDt + SEPARATOR_FIELDS + docTime + SEPARATOR_FIELDS + grossValue + SEPARATOR_FIELDS + totNetDocValue.ToString() + SEPARATOR_FIELDS + tax.ToString() + SEPARATOR_FIELDS + totDisc.ToString() + SEPARATOR_FIELDS + saleQtySum.ToString() + SEPARATOR_FIELDS + transaction_id + SEPARATOR_FIELDS + tax.ToString + SEPARATOR_FIELDS + pmode + SEPARATOR_FIELDS + "2" + SEPARATOR_FIELDS
                    Console.WriteLine("RETURN RECORD")
                Else
                    Console.WriteLine("NITHER SALES RECORD NOR RETURN RECORD AND ARE NOT CONSIDERD IN CANCEL") 'New Case
                End If
