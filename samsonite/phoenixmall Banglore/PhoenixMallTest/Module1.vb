...
Module Module1
    ...
    Sub Main()
        ...
        Try
            ...
            cn = New OleDbConnection(connectString)
            cn.Open()
            Dim return_query As String = "select transnum,itemdatetime,qty,extsellprice,extorigprice as netamt ,(extundiscprice -extsellprice ) as discount,(tax_amt_1+tax_amt_2+tax_amt_3+tax_amt_4+tax_amt_5+tax_amt_6+tax_amt_7+tax_amt_8+tax_amt_9+tax_amt_10+tax_amt_11+tax_amt_12+tax_amt_13+tax_amt_14+tax_amt_15+tax_amt_16 ) from  dbo.Txn_Merchandise_Sale where (convert(datetime,convert(nvarchar,ITEMDATETIME,121)) > '" + ret_response + "' )and txnvoidmod = 0 and extorigprice < 0 order by itemdatetime "
            cmd2 = New OleDbCommand(return_query, cn)
            dr5 = cmd2.ExecuteReader
            Dim rcnt As Integer = 0
            While (dr5.Read)
                rcnt = rcnt + 1
                Dim receiptNo As String = "C-" + dr5.GetValue(0).ToString + "-" + dr5.GetDateTime(1).ToString("yyyy-MM-dd hh':'mm':'ss.fff tt")
    ...
    Private Function serverUpload(ByVal subUrl As String, ByVal params As NameValueCollection)
        ...
        'Dim url As String = serverProtocol + "://192.168.0.157/phoenix_new/public_html/" + serverPath + "/" + subUrl
