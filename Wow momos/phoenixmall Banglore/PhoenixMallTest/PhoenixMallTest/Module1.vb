...
Module Module1
    ...
    Sub Main()
        ...
        Try
            ...
            cn.Open()
            Dim querystr As String = "select RECEIPTID,MAX(CREATEDDATETIME),SUM(NETAMOUNT)*-1,SUM(NETAMOUNTINCLTAX)*-1,SUM(DISCAMOUNT),SUM(TAXAMOUNT) from ax.RETAILTRANSACTIONSALESTRANS where CREATEDDATETIME>'" + response + "'and RECEIPTID like '%BAN001%' and TRANSACTIONSTATUS=0 group by RECEIPTID,CREATEDDATETIME order by CREATEDDATETIME "
            ...
            While dr.Read()
                ...
                Dim query_1 As String = "select ITEMID,QTY,NETAMOUNTINCLTAX*-1,TAXAMOUNT,DISCAMOUNT,PRICE from ax.RETAILTRANSACTIONSALESTRANS where RECEIPTID ='" + receiptNo + "' and TRANSACTIONSTATUS=0 "
                ...
                If totNetDocValue < 0 Then
                    totNetDocValue *= -1
                    Tax *= -1
                    sales_record = receiptNo + SEPARATOR_FIELDS + docDt + SEPARATOR_FIELDS + docTime + SEPARATOR_FIELDS + "" + SEPARATOR_FIELDS + totNetDocValue.ToString() + SEPARATOR_FIELDS + Tax.ToString + SEPARATOR_FIELDS + totDisc.ToString() + SEPARATOR_FIELDS + retqty.ToString + SEPARATOR_FIELDS + transaction_id.ToString + SEPARATOR_FIELDS + "" + SEPARATOR_FIELDS + "" + SEPARATOR_FIELDS + "2" + SEPARATOR_FIELDS

                Else
                    sales_record = receiptNo + SEPARATOR_FIELDS + docDt + SEPARATOR_FIELDS + docTime + SEPARATOR_FIELDS + "" + SEPARATOR_FIELDS + totNetDocValue.ToString() + SEPARATOR_FIELDS + Tax.ToString + SEPARATOR_FIELDS + totDisc.ToString() + SEPARATOR_FIELDS + totqty.ToString + SEPARATOR_FIELDS + transaction_id.ToString + SEPARATOR_FIELDS + "" + SEPARATOR_FIELDS + "" + SEPARATOR_FIELDS + "1" + SEPARATOR_FIELDS
                ...
                Console.WriteLine("invoice count = " + inv_count.ToString)
    ...
    Private Function serverUpload(ByVal subUrl As String, ByVal params As NameValueCollection)
        ...
        'Dim url As String = serverProtocol + "://192.168.0.25/phoenix_new/public_html/" + serverPath + "/" + subUrl
