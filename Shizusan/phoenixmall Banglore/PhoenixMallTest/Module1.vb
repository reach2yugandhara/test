...
Module Module1
    ...
    Sub Main()
        ...
        Dim res_num As String = response1
        'Dim saldate As String = res_date.ToString("yyyy-MM-dd hh':'mm':'ss.fff")
        ...
        Try
            ...
            Dim querystr As String = "select billno,BillMadeDate,BillMadeTime  ,TotalTax ,BillAmount ,(BillAmount - TotalTax ) as bill_amount,Paymode,TotalDiscount,Billamount,ID   from Qfile.TblBillHead_2 where ID > '" + res_num + "'  and Paymode <> 'O'  order by BillMadeDate"
            ...
            While dr.Read()
                ...
                'Dim docTime As String = dr.GetString(1)
                Dim totNetDocValue As Decimal = Convert.ToDecimal(dr.GetValue(8))
                ...
                Dim taxamt As Double = dr.GetValue(3)
                Dim totqty As Double = 0.0
                Dim rettax As Double = 0.0
                Dim retqty As Double = 0.0
                Console.WriteLine("-------------------------------------------------------------")

                'Dim query_1 As String = "select ITEMID,TAXAMOUNT,QTY,DISCAMOUNT,PRICE,NETAMOUNTINCLTAX*-1 from ax.RETAILTRANSACTIONSALESTRANS where (RECEIPTID like '62648001I%' or RECEIPTID like '62648001R%') and store='62648' and CREATEDDATETIME>'" + salres + "' and TRANSACTIONSTATUS=0 and RECEIPTID='" + receiptNo + "'"
                Dim query_1 As String = "SELECT distinct bil.ItemCode ,bil.ItemDescription ,bil.Amount   FROM Qfile.TblPOSRevenue bil  where  bil.BillNo = '" + receiptNo + "'"
                ...
                While dr1.Read()
                    ...
                    lineQuantity = lineQuantity + 1  'Convert.ToDouble(dr1.GetValue(2))
                    ...
                    totqty = lineQuantity
                    line_items += ItemCode.ToString + SEPARATOR_FIELDS + "" + SEPARATOR_FIELDS + "" + SEPARATOR_FIELDS + "" + SEPARATOR_FIELDS + "" + SEPARATOR_FIELDS + discountVal.ToString + SEPARATOR_FIELDS + lineTotal.ToString + SEPARATOR_FIELDS + unitPrice.ToString + SEPARATOR_ITEMFIELDS
                ...
                sales_record = receiptNo + SEPARATOR_FIELDS + docDt + SEPARATOR_FIELDS + "" + SEPARATOR_FIELDS + grossValue.ToString() + SEPARATOR_FIELDS + totNetDocValue.ToString() + SEPARATOR_FIELDS + taxamt.ToString + SEPARATOR_FIELDS + totDisc.ToString() + SEPARATOR_FIELDS + totqty.ToString + SEPARATOR_FIELDS + transaction_id.ToString + SEPARATOR_FIELDS + "" + SEPARATOR_FIELDS + "" + SEPARATOR_FIELDS + "1" + SEPARATOR_FIELDS
                i = (i + 1)
    ...
    Private Function serverUpload(ByVal subUrl As String, ByVal params As NameValueCollection)
        ...
        'Dim url As String = serverProtocol + "://192.168.0.10:8080/phoenix_new/public_html/" + serverPath + "/" + subUrl
