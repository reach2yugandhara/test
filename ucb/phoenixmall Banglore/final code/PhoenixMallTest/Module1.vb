...
Module Module1
        ...
        Try
            ...
            While dr.Read()
                ...
                If (TransType = 5) Then
                    ...
                    Dim querystrcpy As String = "select SUM(LocalAmount) as LocalAmout,SUM(SalesTax) as SalesTax , sum( SalesPrice) as SalesPrice from dbo.SalesReturnTrn where SalesReturnNumber=" + inv_no.ToString + " and SalesReturnYear=" + inv_year.ToString + "and InvoiceType= 31 "
                    ...
                    While drcpy.Read()
                        ...
                        SalesPrice = drcpy.GetDecimal(2)
                        Dim gift_exchng As Integer = 1

                        'Dim query_1 As String = "select invoiceNumber from dbo.SalesReturnTrn where SalesReturnNumber=" + inv_no.ToString + " and InvoiceYear=" + inv_year.ToString + " and invoicetype=31"
                        'cmd = New OleDbCommand(query_1, cn)
                        'dr1 = cmd.ExecuteReader

                        'While dr1.Read()
                        '    gift_exchng = dr1.GetInt32(0)

                        'End While

                        'If (gift_exchng = 0) Then
                        '    tot_tax_amt = 0.0
                        'Else
                        'tot_tax_amt = (tot_net_amt * 14.5) / 114.5
                        'tot_tax_amt = (tot_net_amt * 18) / 118
                        tot_tax_amt = drcpy.GetDecimal(1)
                        'End If
                ...
                Else
                    ...
                    While dr1.Read()
                        ...
                        If (itm_id.Equals("600102ZNGAAP00")) Then
                            Continue While
                        Else
                            Dim invoice_type As Byte = dr1.GetByte(0)
                ...
                If (TransType = 5) Then
                    ...
                    sales_record = doc_no.ToString + SEPARATOR_FIELDS + invDateFormated.ToString("yyyy-MM-dd HH:mm:ss") + SEPARATOR_FIELDS + "" + SEPARATOR_FIELDS + SalesPrice.ToString() + SEPARATOR_FIELDS + tot_net_amt.ToString() + SEPARATOR_FIELDS + tot_tax_amt.ToString() + SEPARATOR_FIELDS + tot_disc.ToString() + SEPARATOR_FIELDS + tot_sold_qty.ToString() + SEPARATOR_FIELDS + transaction_id.ToString() + SEPARATOR_FIELDS + "" + SEPARATOR_FIELDS + "" + SEPARATOR_FIELDS + salesTrnType.ToString + SEPARATOR_FIELDS
                Else
                    ...
                    sales_record = receipt_no.ToString + SEPARATOR_FIELDS + invDateFormated.ToString("yyyy-MM-dd HH:mm:ss") + SEPARATOR_FIELDS + "" + SEPARATOR_FIELDS + tot_net_amt.ToString() + SEPARATOR_FIELDS + Invoice_amt.ToString() + SEPARATOR_FIELDS + tot_tax_amt.ToString() + SEPARATOR_FIELDS + tot_disc.ToString() + SEPARATOR_FIELDS + tot_sold_qty.ToString() + SEPARATOR_FIELDS + transaction_id.ToString() + SEPARATOR_FIELDS + "" + SEPARATOR_FIELDS + "" + SEPARATOR_FIELDS + salesTrnType.ToString + SEPARATOR_FIELDS
    ...
    Private Function serverUpload(ByVal subUrl As String, ByVal params As NameValueCollection)
        ...
        'Dim url As String = serverProtocol + "://192.168.0.28/phoenix_new/public_html/" + serverPath + "/" + subUrl
