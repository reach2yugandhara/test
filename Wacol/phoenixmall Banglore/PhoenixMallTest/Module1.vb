...
Module Module1
    ...
    Sub Main()
        ...
        Try
            ...
            Dim return_query As String = "select transnum,max(itemdatetime),SUM(qty),SUM(extsellprice),SUM(extorigprice) as netamt ,SUM(extundiscprice -extsellprice ) as discount,SUM(tax_amt_1+tax_amt_2+tax_amt_3+tax_amt_4+tax_amt_5+tax_amt_6+tax_amt_7+tax_amt_8+tax_amt_9+tax_amt_10+tax_amt_11+tax_amt_12+tax_amt_13+tax_amt_14+tax_amt_15+tax_amt_16 ) from  dbo.Txn_Merchandise_Sale where (convert(datetime,convert(nvarchar,ITEMDATETIME,121)) > '" + ret_response + "' ) and extsellprice <0 and txnvoidmod = 0 group by transnum "
